...
const AddressMappings = () => {
  ...
  const loadMappings = async () => {
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/address-mappings', {
        headers: {
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      const response = await fetch('http://localhost:5000/api/admin/address-mappings/upload', {
        method: 'POST',
        headers: {
          'Authorization': `Bearer ${token}`
        },
  ...
  const downloadTemplate = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/address-mappings/template', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const downloadCurrentData = async () => {
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/address-mappings/export', {
        headers: {
          'Authorization': `Bearer ${token}`
        }
      });
  ...
  const clearAllMappings = async () => {
    ...
    try {
      const token = localStorage.getItem('token');
      const response = await fetch('http://localhost:5000/api/admin/address-mappings/clear', {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${token}`,
          'Content-Type': 'application/json'
